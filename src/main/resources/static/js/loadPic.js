...
function loadPage(pageNum,pageSize){
	...
	for (var i = 0; i < pageSize && i < picList.length; i++) {
        ...
        div += '<div id="pic'+img.id+'" class="item">';
        //div += '<div id="div'+id+'" class="item" style="padding: ' + imgPadding + 'px;margin: 0px;">';
        div += '<div class="animate-box bounceIn animated">';
        div += '<a href="'+urlBig+'" class="image-popup fh5co-board-img" title="'+img.title+'">';
        div += '<img src="'+urlSmall+'" alt="'+img.title+'">';
        div += '</a>';
        div += '</div>';
        div += '<div class="fh5co-desc">'+img.title+'</div>';
        div += '</div>';

		$('#fh5co-board').append(div);
	}
	//初始化baguetteBox
